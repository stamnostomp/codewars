...
module SumOfPowersOf2 where

powers :: Int -> [Int]
powers n = findCombinations n (squaresLessThan n)
  where
    squaresLessThan :: Int -> [Int]
    squaresLessThan n = takeWhile (< n) [x ^ 2 | x <- [1 ..]]

    findCombinations :: Int -> [Int] -> [Int]
    findCombinations n squares = findCombinationsHelper n squares []

    findCombinationsHelper :: Int -> [Int] -> [Int] -> [Int]
    findCombinationsHelper _ [] _ = []
    findCombinationsHelper n (x : xs) current
      | n == 0 = current
      | n < 0 = []
      | otherwise = case findCombinationsHelper (n - x) xs (x : current) of
          [] -> findCombinationsHelper n xs current
          result -> result
